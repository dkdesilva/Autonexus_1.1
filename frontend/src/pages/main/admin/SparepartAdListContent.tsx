...
// Helper to decode JWT and get admin_id
...
const SparepartAdListContent = () => {
  const [data, setData] = useState<SparepartAdData[]>([]);
  const [search, setSearch] = useState("");

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get<SparepartAdData[]>(
          "http://localhost:5000/api/sparepart/all"
        );
        setData(res.data);
      } catch (err) {
  ...
  const handleApproval = async (
    ...
    try {
      ...
      toast.success(`Ad ${status.toLowerCase()} successfully.`);
    } catch (err) {
          ...
          <table className="min-w-max border table-auto">
            <thead>
              <tr className="border-b text-left text-black">
                ...
                <th className="py-2 px-4">Compatibility</th>
            ...
            <tbody>
              {filtered.map((item) => (
                ...
                >
                  ...
                  <td className="py-2 px-4">{item.model_compatibility}</td>
...
export default SparepartAdListContent;
