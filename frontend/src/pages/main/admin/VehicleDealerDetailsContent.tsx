...
const VehicleDealerDetailsContent = () => {
  const [data, setData] = useState<CarDealerUserData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await axios.get('http://localhost:5000/api/users/cardealers', {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h2 className="text-xl font-semibold mb-4 text-black">Car Dealer Details</h2>
          ...
          <tbody>
            {filtered.map((item) => (
              <tr key={item.user_id} className="border-b hover:bg-gray-50 text-black">
                ...
                <td className="py-2 px-4">{item.dealer_id}</td>
...
export default VehicleDealerDetailsContent;
