...
const RejectedSpareListings: React.FC<Props> = ({ sparePartListings, animationProps }) => {
  const rejectedParts = sparePartListings.filter(part => part.approval_status === 'Rejected');

  return (
    <motion.div key="rejected-spares" {...animationProps}>
      <h2 className="text-xl font-semibold mb-6">Rejected Spare Parts</h2>
      ...
      {rejectedParts.length === 0 ? (
        <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-8 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            You currently have no rejected spare part listings.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {rejectedParts.map((part) => (
            <div
              key={part.ad_id}
...
export default RejectedSpareListings;
